...
class RecipeLabFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        adapter = RecipeAdapter(allRecipes.toMutableList(), requireContext())
        recycler.adapter = adapter

        val searchBox = view.findViewById<EditText>(R.id.search_recipes)
            ...
            override fun onTextChanged(s: CharSequence?, start: Int, before: Int, count: Int) {
                ...
                val filtered = allRecipes.filter {
                    it.name.lowercase().contains(query) ||
                            it.millet.lowercase().contains(query)
                }
